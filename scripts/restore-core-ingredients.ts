...
async function runRecovery(strapi: Core.Strapi) {
    console.log('[RECOVERY] Starting core ingredient restoration to Strapi...');
    ...
    for (const item of ingredients) {
        ...
        try {
            ...
            const existing = await strapi.documents('api::ingredient.ingredient' as any).findMany({
                filters: { slug: slug }
            });

            if (existing.length > 0) {
                // Update
                await strapi.documents('api::ingredient.ingredient' as any).update({
                ...
                // Create
                await strapi.documents('api::ingredient.ingredient' as any).create({
                    data: {
                        name: item.name,
                        slug: slug,
                        kcal: item.kcal,
            ...
            console.error(`[RECOVERY] Failed to restore ${item.name}:`, err.message);
        }
    }
    
    console.log(`[RECOVERY] Restored ${count} core ingredients.`);
}
